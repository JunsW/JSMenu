...
//                print("Move at index: \(index), seletedIndex: \(selectedIndex), endingIndex: \(panEndingIndex)")
        ...
        print(addButtonIndex)
        if index.row < deleteButtonIndex {
            // 交换Cell 交换之后 indexPath也会更新 所以数据原也要更新
            dynamicData.swapAt(beginningIndex.row, index.row)
    ...
    private func panEnded(gesture: UIPanGestureRecognizer)  {
        ...
        UIView.animate(withDuration: 0.3) {
            self.animationCell?.center = (self.menuCollection.cellForItem(at: self.panEndingIndex ?? self.selectedIndex!)?.center)! //如果没有结束index说明没有交换过，居中到起始cell
